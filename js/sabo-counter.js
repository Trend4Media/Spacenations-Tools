...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        console.log('💥 Sabo-Counter wird initialisiert...');
        ...
        if (window.AuthAPI && isDashboardMode) {
            ...
            window.AuthAPI.onAuthStateChange(async (user, userDataFromAuth) => {
                if (user && userDataFromAuth) {
                    ...
                    console.log('✅ Sabo-Counter für User geladen:', userData.username);
                    ...
                    console.log('❌ Kein User - Standard Sabo-Counter Modus');
                    enableStandardFeatures();
                }
            });
        } else {
            console.log('⚔️ Standard Sabo-Counter Modus (ohne Dashboard)');
            enableStandardFeatures();
        }
...
function enableStandardFeatures() {
    ...
    console.log('⚔️ Standard-Features aktiviert');
}
...
async function saveSabotageData() {
    ...
    try {
        ...
        console.log('✅ Sabotage gespeichert:', docRef.id);
...
async function updateDashboardSabotageStats() {
    try {
        ...
        console.log('📊 Dashboard-Sabotage-Statistiken aktualisiert');
...
async function loadRecentSabotages() {
    try {
        ...
        console.log('📝 Letzte Sabotagen geladen:', sabotagesQuery.size);
...
function testSabotageParser(testText) {
    console.log('🧪 Teste Sabotage-Parser...');
...
Forschungszentrum 1
    ...
    console.log('Parser-Ergebnis:', result);
...
console.log('💥 Sabo-Counter JavaScript vollständig geladen');
console.log('💡 Keyboard-Shortcuts: Strg+S (Speichern), Strg+Enter (Berechnen), Strg+D (Dashboard), Strg+Delete (Löschen)');
console.log('🔧 API verfügbar: window.SabotageCounterAPI');
console.log('🧪 Debug verfügbar: window.saboDebug');

// Auto-Test beim Laden (nur im Development)
setTimeout(() => {
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        console.log('🧪 Development-Modus erkannt - Auto-Test verfügbar');
